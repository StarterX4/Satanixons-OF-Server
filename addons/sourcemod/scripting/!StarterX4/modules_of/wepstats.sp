...
public void BuildWepMenus()
{
	...
	hWepMenus[2][2].AddItem("772", "Escape Plan");
	...
	hWepMenus[7][1].AddItem("23", "Nailgun");
...
stock void GetWeaponStat(int idx, char[] buffer, int maxlen, TFClassType class = TFClass_Unknown)
{
	...
	switch (idx)
	{
		...
		case 448:strcopy(buffer, maxlen, "Soda Popper: Bonus damage to burning players");
		...
		case 228, 1085:strcopy(buffer, maxlen, "Black Box: Receive 2 crits when the medic healing you dies.");
		...
		case 19, 206, 1007, 15077, 15079, 15091, 15092, 15116, 15117, 15142, 15158:strcopy(buffer, maxlen, "Grenade Launcher: Reduced damage when blast jumping while active.");
		case 308:strcopy(buffer, maxlen, "Loch-n-Load");
		case 996:strcopy(buffer, maxlen, "Loose Cannon: Reduced damage when blast jumping while active.");
		case 1151:strcopy(buffer, maxlen, "Iron Bomber: Reduced damage when blast jumping while active.");
		...
		case 17, 204:strcopy(buffer, maxlen, "Syringe Gun: Gain 5% uber on hit.");
		case 36:strcopy(buffer, maxlen, "Blutsauger: Gain 1% uber on hit, can overheal.");
		case 412:strcopy(buffer, maxlen, "Overdose: Gain 1% uber on hit. Global speed boost.");
		...
		case 56, 1005, 1092:strcopy(buffer, maxlen, "Huntsman: +50% damage. Double ammo reserve. Full crits.");
		...
		case 460:strcopy(buffer, maxlen, "Enforcer: Receive no fall damage.");
		...
		case 22, 209, 294, 15013, 15018, 15035, 15041, 15046, 15056, 15060, 15061, 15100, 15101, 15102, 15126, 15148, 30666:FormatEx(buffer, maxlen, "Pistol: Receive %scrits.", class == TFClass_Scout ? "mini-" : "full ");
		...
		case 449:strcopy(buffer, maxlen, "Winger: Increased damage boost.");
		case 773:strcopy(buffer, maxlen, "PBPP: Take no fall damage.");
		case 812, 833:strcopy(buffer, maxlen, "Flying Guillotine: Gains full crits; gain a speed boost on hit; bonus primary ammo.");
		case 222, 1121:strcopy(buffer, maxlen, "Mad Milk: Healing grants overheal.");
		...
		case 129, 1001:strcopy(buffer, maxlen, "Buff Banner");
		...
		case 159:strcopy(buffer, maxlen, "Dalokohs Bar: Health boost is upped to 500");
		...
		case 411:strcopy(buffer, maxlen, "Quick-Fix: When healing, target and you gain a defence buff.");
		...
		case 128:strcopy(buffer, maxlen, "Equalizer: +20% faster swing speed.");
		...
		case 775:strcopy(buffer, maxlen, "Escape Plan");
		case 2, 192:strcopy(buffer, maxlen, "Fire Axe");
		case 153:strcopy(buffer, maxlen, "Homewrecker: Repair/upgrade friendly buildings.");
		case 214:strcopy(buffer, maxlen, "Powerjack: Gain health on hit, speed boost active at all times, increased damage vuln.");
		case 326:strcopy(buffer, maxlen, "Back Scratcher");
		case 348:strcopy(buffer, maxlen, "Sharpened Volcano Fragment: Take no fall damage.");
		...
		case 239, 1084, 1100:strcopy(buffer, maxlen, "GRU: Go 2.5x while active. Take 30 damage/sec.");
