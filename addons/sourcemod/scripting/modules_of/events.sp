public void OF_OnPlayerSpawned(int client)
{
	...
	{
		if (gamemode.iRoundState == StateRunning && !(gamemode.iSpecialRound & (ROUND_HVH|ROUND_SURVIVAL)) && !CheckCommandAccess(player.index, "sm_asdcdfc", ADMFLAG_GENERIC, true))
		{
		...
		{
			TF2_SetPlayerClass(player.index, TFClass_Mercenary);
			TF2_RegeneratePlayer(player.index);
			OF_InitClass(player.index);
		}
		...
		if (!player.bIsBoss && !player.bIsMinion && gamemode.iRoundState == StateStarting || gamemode.iRoundState == StateEnding)
		{
			TeleportToSpawn(player.index, RED);
...
public Action RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (gamemode.iRoundState == StateStarting && gamemode.CountBosses(false))
		return Plugin_Continue;
...
//	gamemode.bMedieval = (FindEntityByClassname(-1, "tf_logic_medieval") != -1 || tf_medieval.BoolValue);
	...
	{
		SDKHook(EntRefToEntIndex(AttachParticle(boss.index, GetClientTeam(boss.index) == BLU ? "teleporter_blue_exit" : "teleporter_red_exit", .remove = false)), SDKHook_SetTransmit, BVBGlowTransmit);
		mp_teams_unbalance_limit.SetInt(1);
		...
		for (i = 0; i < numbosses; ++i)
		{
			...
			if (IsClientValid(player.index))
			{
				...
				SDKHook(EntRefToEntIndex(AttachParticle(player.index, !flip ? "teleporter_red_exit" : "teleporter_blue_exit", .remove = false)), SDKHook_SetTransmit, BVBGlowTransmit);
				toggle(flip);
			}
			else break;
		}
	...
	else
	{
		...
		{
			if (!IsClientInGame(i) || GetClientTeam(i) <= int(TFTeam_Spectator))
				continue;
	...
	SetPawnTimer(RoundStartPost, 5.1, gamemode.iRoundCount);		// in handler.sp
...
//		boss.flMusicTime = 0.0;
		SetEntityMoveType(i, MOVETYPE_WALK);
		if (!boss.bIsBoss) {
			if (GetClientTeam(i) != gamemode.iOtherTeam && GetClientTeam(i) > int(TFTeam_Spectator) && !(gmflags & ROUND_HVH))	// For good measure!
	...
	for (i=MaxClients ; i ; --i) {		// Loop again for bosses only
		...
		if (boss.iMaxHealth < 1000 && bosscount == 1)
			boss.iMaxHealth = 1000;
		else if ( boss.iMaxHealth > 1500 && bosscount > 1 )
...
public Action PointCapture(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (gamemode.iCaptures >= cvarVSH2[MultiCapAmount].IntValue || (GetLivingPlayers(gamemode.iOtherTeam) == 1 && iCapTeam == gamemode.iOtherTeam))
	{
...
public Action OnJoinTeam(Event event, const char[] name, bool db)
{
	event.BroadcastDisabled = true;
...
public Action OnPlayerJump(Event event, const char[] name, bool db)
{
	int client = event.GetInt("playerid");
	if (TF2_GetPlayerClass(client) != TFClass_Mercenary)
		event.BroadcastDisabled = true;
}